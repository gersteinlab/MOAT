...
// 	}
	...
	if (argc < 5) {
		fprintf(stderr, "Usage: moatsim_find_allele [FASTA dir] [variant file] [allele file] [output file]. Exiting.\n");
		return 1;
	} else {
		fasta_dir = string(argv[1]);
		vfile = string(argv[2]);
		allele_file = string(argv[3]);
...
// 	return 0;
	...
	/* Import allele columns */
	
	// Vectors to keep track of how many allele groups we have
	// 0: A; 1: C; 2: G; 3: T
	vector<vector<int> > allele_count;
	for (int i = 0; i < 4; i++) {
		vector<int> temp (4,0);
		allele_count.push_back(temp);
	}
	// string allele_file = "/home/fas/gerstein/ll426/scratch/data/larva-by-type/Liver.whole.sorted.allele.txt";
	FILE *allele_file_ptr = fopen(allele_file.c_str(), "r");
	char linebuf_cstr[STRSIZE];
	while (fgets(linebuf_cstr, STRSIZE, allele_file_ptr) != NULL) {
		string linebuf = string(linebuf_cstr);
		// linebuf.erase(linebuf.find_last_not_of(" \n\r\t")+1);
		
		string ref;
		size_t ws_index = linebuf.find_first_of("\t\n");
		ref = linebuf.substr(0, ws_index);
		linebuf = linebuf.substr(ws_index+1);
		...
		allele_count[refnum][altnum] += 1;
	}
	// Check feof of allele file
	if (feof(allele_file_ptr)) { // We're good
		fclose(allele_file_ptr);
	} else { // It's an error
		char errstring[STRSIZE];
		sprintf(errstring, "Error reading from %s", allele_file.c_str());
		perror(errstring);
		return 1;
	}
	...
	/* Assign new alleles and output */
	// string outfile;
	FILE *outfile_ptr = fopen(outfile.c_str(), "w");
	for (unsigned int i = 0; i < var_array.size(); i++) {
		string chr = var_array[i][0];
		string pos = var_array[i][1];
