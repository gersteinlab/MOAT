...
Additionally, MOAT-v can provide the functional impact scores of the permuted variant datasets from Funseq2, another software tool from the Gerstein lab that evaluates the functional impact of single nucleotide variants (funseq2.gersteinlab.org). [funseq_mode] can be set to 'n' for (n)o Funseq computation, or 's' for (s)tatic Funseq computation, or 'd' for (d)ynamic Funseq computation.

The static mode retrieves Funseq scores from a file with precomputed Funseq scores over the entire human genome. Each position is assigned the maximum Funseq score possible for that position. This mode is considerably faster than dynamic mode, although the Funseq output will not be as precise as using the actual allele information. The precomputed Funseq score file can be found on the MOAT website: [].

The dynamic mode computes Funseq scores with a local Funseq2 installation. Hence, additional setup is required, and the complete Funseq computation will take longer than the static mode. However, all Funseq output will be exact, rather than approximate. Funseq can be downloaded from funseq2.gersteinlab.org.
...
Additionally, p_value_emp can derive the Funseq scores of the annotations in the [annotation file], as implied by the variants in the [variant file]. [funseq option] can be set to 'n' for (n)o Funseq, or 'o' for (o)bserved variants only, or 'p' for (p)ermuted variants. The observed variants mode computes Funseq scores for the [variant file] variants, sums them over the annotations in the [annotation file], and adds them to the [output file]. The permuted variants mode does the same thing as observed variants mode, and also uses the Funseq scores for the permuted variant datasets (must have used Funseq computation in MOAT-v) to calculate a p-value significance for the annotations' Funseq score, which are calculated and reported similar to the mutation burden p-value.

[funseq mode] determines how Funseq scores are computed. It can be 's' for (s)tatic mode, which derives the scores from a file with precomputed Funseq scores over the entire human genome. Each position is assigned the maximum Funseq score possible for that position. This mode is considerably faster than dynamic mode, although the Funseq output will not be as precise as using the actual allele information. The precomputed Funseq score file can be found on the MOAT website: [].
